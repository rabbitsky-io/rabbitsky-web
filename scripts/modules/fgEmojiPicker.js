var FgEmojiPicker = function(options) {
    ...
    this.init = () => {
        ...
        this.fetchObj = this.emojiPicker();
    ...
    this.functions = {
        ...
        search(e) {
            ...
            this.emojiItems.filter(emoji => {
                var emojiTags = emoji.getAttribute('data-name').split(',');
                var found = false;

                for(var i = 0; i < emojiTags.length; i++) {
                    if(emojiTags[i].indexOf(val) >= 0) {
                        found = true;
                        break;
                    }
                }
        ...
        emitEmoji(e) {
            ...
            if (el.tagName.toLowerCase() == 'li' && el.firstElementChild && el.firstElementChild.className.includes('fg-emoji-picker-item')) {
            ...
            if (el.tagName.toLowerCase() == 'li' && el.className.includes('fg-emoji-picker-item')) {
                e.preventDefault();
        ...
        openEmojiSelector(e) {
            ...
            if (el) {
                ...
                this.fetchObj.then(emojiPicker => {
                    ...
                    document.body.querySelector('.fg-emoji-picker').onclick =  function(e) {
                        ...
                        if (filterLlnk && filterLlnk.closest('li') && filterLlnk.closest('li').getAttribute('data-index')) {
                            ...
                            scrollTo(emojiBody, emojiBody.querySelector(`#${listIndex}`).offsetTop - 60);
                        }
    ...
    this.emojiPicker = () => {
        ...
        const fetchData = fetch(`${this.variable.dir}emoji.json`)
            ...
            for (const key in object) {
                if (object.hasOwnProperty(key)) {
                    ...
                    categories.forEach(item => {
                        innerLists += `<li data-name="${item.keywords.join(",").toLowerCase()}"><a class="fg-emoji-picker-item" title="${item.keywords.join(",").toLowerCase()}" data-name="${item.keywords.join(",").toLowerCase()}" data-code="${item.code}" href="${item.emoji}">${item.emoji}</a></li>`;
                    })
